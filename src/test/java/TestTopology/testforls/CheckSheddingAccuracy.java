...
public class CheckSheddingAccuracy {
    ...
    public void calcODaccuracy() {
        ...
        sheddatas.removeAll(nap);
        for (int i =0; i<fulldatas.size(); i++) {
            if (fulldatas.get(i).equals("END")) {
                System.out.print(i+"~"+fulldatas.get(i)+"   ");
            }
        }
        System.out.println();
        for (int i =0; i<sheddatas.size(); i++) {
            if (sheddatas.get(i).equals("END")) {
        ...
        while (!fulldatas.isEmpty() || !sheddatas.isEmpty()) {
            ...
            if (len+1 < fulldatas.size()) {
                fulldatas = fulldatas.subList(len + 1, fulldatas.size());
            } else {
                fulldatas.clear();
            }
            ...
            if (len+1 < sheddatas.size()) {
                sheddatas = sheddatas.subList(len + 1, sheddatas.size());
            } else {
                sheddatas.clear();
            }
            ...
            Map<Double, Integer> precisionResult = new HashMap<>();
            Map<Double, Integer> recallResult = new HashMap<>();
            int right;String[] result;double precision;double recall;double f1;int temp;int count = 0;
            HashMap<String, String[]> realmap = new HashMap<>();
            while (iteratorFull.hasNext()) {
                HashMap<String, String[]> fullmap = fix2((String) iteratorFull.next());
                for (String str : fullmap.keySet()) {
            ...
            //System.out.println(f1Result.keySet());
            int total = f1Result.values().stream().mapToInt(Number::intValue).sum();
           // System.out.println("count="+count+" failure="+failure+" miss="+miss+" total: "+total);
