...
import TestTopology.fp.SentenceSpout;
 ...
 */
public class FPtop implements Constant {
    ...
    public static void main(String[] args) throws Exception {
        ...
        int checktype = ConfigUtil.getInt(conf, "tz", 2);
//Integer.valueOf((Integer) conf.get("test.shedding.or.not"));
        TopologyBuilder builder = null;
        if (checktype == 0) {
            builder = new TopologyBuilder();
            System.out.println("no static map fp origin storm");
        } else if (checktype == 1) {
            builder = new ResaTopologyBuilder();
            System.out.println("no static map fp origin drs");
        } else if (checktype == 2) {
            builder = new SheddingResaTopologyBuilder();
            System.out.println("no static map fp shedding drs");
        }
        ...
        System.out.println(checktype+"mudedajihe: "+generator_mu+"~~~"+detector_mu+"~~~"+reporter_mu);
        builder.setSpout("input", new SentenceSpout(host, port, queue), ConfigUtil.getInt(conf, "fp.spout.parallelism", 1));

        builder.setBolt("generator", new FPtopGenerator(() -> (long) (-Math.log(Math.random()) * 1000.0 / generator_mu)), ConfigUtil.getInt(conf, "fp.generator.parallelism", 1))
                .shuffleGrouping("input")
                .setNumTasks(ConfigUtil.getInt(conf, "fp.generator.tasks", 1));
        builder.setBolt("detector", new FPtopDetector(() -> (long) (-Math.log(Math.random()) * 1000.0 / detector_mu)), ConfigUtil.getInt(conf, "fp.detector.parallelism", 1))
                .directGrouping("generator")
                //.directGrouping("detector", FEEDBACK_STREAM)
                .setNumTasks(ConfigUtil.getInt(conf, "fp.detector.tasks", 1));

        builder.setBolt("reporter", new FPtopReporter(() -> (long) (-Math.log(Math.random()) * 1000.0 / reporter_mu)), ConfigUtil.getInt(conf, "fp.reporter.parallelism", 1))
        ...
        if (ConfigUtil.getBoolean(conf, "fp.metric.resa", true)) {
            if (checktype == 1) {
                ...
                System.out.println("no static map fp ResaMetricsCollector is registered");
            } else if (checktype == 2) {
                resaConfig.addSheddingSupport();
                resaConfig.put(ResaConfig.REBALANCE_WAITING_SECS, 0);
                System.out.println("no static map fp shedding ResaMetricsCollector is registered");
            }
        }
